class PortfolioApp{constructor(){this.isLoading=true;this.scrollProgress=document.querySelector(".scroll-progress");this.loadingElement=document.getElementById("loading");this.mainContent=document.getElementById("main-content");this.dockContainer=document.getElementById("dock-container");this.portfolioData=window.portfolioData;this.projectOverrides=window.projectOverrides;this.THREE=window.THREE;this.gsap=window.gsap;this.Veloxi=window.Veloxi;this.init()}async init(){try{this.initializePerformanceMonitoring();this.initializeAdaptiveLoading();await this.loadPortfolioData();this.hideLoading();setTimeout(()=>{this.initializeDock();this.initializeDockHighlighting();this.initializeFadeInAnimations();this.initializeExperienceAnimations();this.initializeScrollProgress()},100)}catch(error){console.error("Failed to initialize portfolio:",error);this.showError("Failed to initialize portfolio. Please check the console for details.")}}async loadPortfolioData(){try{if(!this.portfolioData||!this.portfolioData.personal){throw new Error("Portfolio data not found or invalid")}const githubProjects=await this.fetchGitHubProjects(this.portfolioData.personal.githubUsername);const mergedProjects=this.mergeProjects(githubProjects,this.projectOverrides,this.portfolioData.personal.hideForks||false);this.renderPersonalInfo(this.portfolioData.personal||{});this.renderSocialLinks(this.portfolioData.socialLinks||[]);this.renderSkills(this.portfolioData.skills||[]);this.renderExperience(this.portfolioData.experience||[]);this.renderExtracurriculars(this.portfolioData.extracurriculars||[]);this.renderProjects(mergedProjects);this.renderCertificates(this.portfolioData.certificates||[]);this.renderNavigation(this.portfolioData.navigation||[])}catch(error){console.error("Failed to load portfolio data:",error);throw error}}async fetchGitHubProjects(username,token=null){try{const headers=token?{Authorization:`token ${token}`}:{};const response=await fetch(`https://api.github.com/users/${username}/repos`,{headers:headers});if(!response.ok){if(response.status===403){throw new Error("GitHub API rate limit exceeded. Please try again later.")}throw new Error(`GitHub API error! status: ${response.status}`)}const repos=await response.json();return repos.map(repo=>({name:repo.name,description:repo.description||"",createdAt:repo.created_at,updatedAt:repo.updated_at,githubUrl:repo.html_url,liveUrl:repo.homepage||null,technologies:repo.topics||[],featured:false,thumbnail:null,fork:repo.fork}))}catch(error){console.error("Failed to fetch GitHub projects:",error);this.showError(error.message);return[]}}mergeProjects(githubProjects,overrides,hideForks){const overrideMap=new Map(overrides.projects.map(p=>[p.name,p]));const hiddenSet=new Set(overrides.hiddenProjects);return githubProjects.filter(project=>!hiddenSet.has(project.name)&&(!hideForks||!project.fork)&&(!overrides.hideArchived||!project.archived)).map(project=>{const override=overrideMap.get(project.name)||{};return{...project,description:override.description||project.description,liveUrl:override.liveUrl||project.liveUrl,technologies:override.technologies||project.technologies,featured:override.featured!==undefined?override.featured:project.featured}})}renderPersonalInfo(personalData){const mainTitle=document.getElementById("main-title");mainTitle.innerHTML=`
        ...
        `;document.getElementById("bio-text").innerHTML=personalData.bio||"";document.getElementById("profile-image").src=personalData.profileImage||"/placeholder.svg";document.getElementById("profile-image").alt=`${personalData.name||"Profile"} Picture`;const emailLink=document.getElementById("email-link");emailLink.href=`mailto:${personalData.email||""}`;emailLink.textContent=personalData.email||"N/A"}renderSocialLinks(socialLinks){const container=document.getElementById("social-links");container.innerHTML=(socialLinks||[]).map(link=>`
        ...
        `).join("")}renderSkills(skills){if(!this.THREE){console.warn("Three.js not loaded, skipping 3D skills visualization");return}const scn=new this.THREE.Scene;const cam=new this.THREE.PerspectiveCamera(75,1,.1,1e3);const rndr=new this.THREE.WebGLRenderer({antialias:true,alpha:true});const grp=new this.THREE.Group;const ldr=new this.THREE.TextureLoader;const sps=[];const r=27;const s=6;const c=document.querySelector(".skill-tag-cloud");if(!c)return;const getContainerSize=()=>{const containerWidth=c.clientWidth||window.innerWidth;const containerHeight=c.clientHeight||window.innerHeight*.6;const size=Math.min(containerWidth,containerHeight,500);return{width:size,height:size}};let{width,height}=getContainerSize();const container=document.createElement("div");container.style.cssText=`
            position: absolute;
            top: 10px;
            left: 10px;
            ...
            pointer-events: none;
        `;c.appendChild(container);const tooltip=document.createElement("div");tooltip.style.cssText=`
            color: white;
            font-size: 12px;
            display: none;
            line-height: 1.4;
        `;container.appendChild(tooltip);rndr.setSize(width,height);rndr.domElement.style.cssText=`
            display: block;
            max-width: 100%;
            height: auto;
        `;c.appendChild(rndr.domElement);rndr.setClearColor(0,0);cam.aspect=width/height;cam.updateProjectionMatrix();let hoveredSprite=null;let selectedSprite=null;let isDragging=false;let previousX=0;let previousY=0;let autoRotate=true;let lastTouchTime=0;const handleResize=()=>{const newSize=getContainerSize();width=newSize.width;height=newSize.height;rndr.setSize(width,height);cam.aspect=width/height;cam.updateProjectionMatrix();updateCanvasRect()};window.addEventListener("resize",handleResize);(skills||[]).forEach((skill,i)=>{ldr.load(skill.icon||"/placeholder.svg",t=>{const phi=Math.acos(-1+2*i/skills.length);const theta=Math.sqrt(skills.length*Math.PI)*phi;const sp=new this.THREE.Sprite(new this.THREE.SpriteMaterial({map:t}));sp.scale.set(s,s,1);sp.position.set(r*Math.cos(theta)*Math.sin(phi),r*Math.sin(theta)*Math.sin(phi),r*Math.cos(phi));sp.userData={name:skill.name||"Skill",category:skill.category||"General",originalScale:s};grp.add(sp);sps.push(sp)},undefined,error=>{console.warn(`Failed to load skill icon: ${skill.icon}`,error)})});scn.add(grp);cam.position.z=50;const ry=new this.THREE.Raycaster;let canvasRect=rndr.domElement.getBoundingClientRect();function updateCanvasRect(){canvasRect=rndr.domElement.getBoundingClientRect()}window.addEventListener("scroll",updateCanvasRect);function getNormalizedCoords(clientX,clientY){return{x:2*(clientX-canvasRect.left)/canvasRect.width-1,y:2*-(clientY-canvasRect.top)/canvasRect.height+1}}function isWithinCanvas(clientX,clientY){return clientX>=canvasRect.left&&clientX<=canvasRect.right&&clientY>=canvasRect.top&&clientY<=canvasRect.bottom}const handleInteractionStart=(clientX,clientY)=>{if(!isWithinCanvas(clientX,clientY))return false;isDragging=true;previousX=clientX;previousY=clientY;const coords=getNormalizedCoords(clientX,clientY);ry.setFromCamera(coords,cam);const it=ry.intersectObjects(sps)[0];if(it){selectedSprite=it.object;autoRotate=false;const targetPos=it.object.position.clone().normalize().multiplyScalar(20);if(this.gsap){this.gsap.to(cam.position,{x:targetPos.x,y:targetPos.y,z:targetPos.z,duration:.5,ease:"power2.out"});this.gsap.to(it.object.scale,{x:s*1.5,y:s*1.5,duration:.3})}}else if(selectedSprite){selectedSprite=null;if(this.gsap){this.gsap.to(cam.position,{x:0,y:0,z:50,duration:.5,ease:"power2.out"});sps.forEach(sp=>{this.gsap.to(sp.scale,{x:sp.userData.originalScale,y:sp.userData.originalScale,duration:.3})})}}return true};const handleInteractionMove=(clientX,clientY)=>{if(!isWithinCanvas(clientX,clientY))return;const coords=getNormalizedCoords(clientX,clientY);ry.setFromCamera(coords,cam);const intersects=ry.intersectObjects(sps);if(hoveredSprite&&(!intersects.length||intersects[0].object!==hoveredSprite)){if(this.gsap){this.gsap.to(hoveredSprite.scale,{x:hoveredSprite.userData.originalScale,y:hoveredSprite.userData.originalScale,duration:.2})}hoveredSprite=null;tooltip.style.display="none"}if(intersects.length>0&&intersects[0].object!==hoveredSprite){hoveredSprite=intersects[0].object;if(this.gsap){this.gsap.to(hoveredSprite.scale,{x:hoveredSprite.userData.originalScale*1.2,y:hoveredSprite.userData.originalScale*1.2,duration:.2})}tooltip.style.display="block";tooltip.innerHTML=`<strong>${hoveredSprite.userData.name}</strong><br>${hoveredSprite.userData.category}`}if(isDragging){const deltaX=clientX-previousX;const deltaY=clientY-previousY;grp.rotation.y+=deltaX*.005;grp.rotation.x+=deltaY*.005;previousX=clientX;previousY=clientY}};const handleInteractionEnd=()=>{isDragging=false};rndr.domElement.addEventListener("mousedown",e=>{e.preventDefault();handleInteractionStart(e.clientX,e.clientY)});window.addEventListener("mousemove",e=>{handleInteractionMove(e.clientX,e.clientY)});window.addEventListener("mouseup",handleInteractionEnd);rndr.domElement.addEventListener("touchstart",e=>{e.preventDefault();const touch=e.touches[0];const currentTime=Date.now();if(currentTime-lastTouchTime<300){autoRotate=!autoRotate;if(selectedSprite){selectedSprite=null;if(this.gsap){this.gsap.to(cam.position,{x:0,y:0,z:50,duration:.5,ease:"power2.out"});sps.forEach(sp=>{this.gsap.to(sp.scale,{x:sp.userData.originalScale,y:sp.userData.originalScale,duration:.3})})}}}lastTouchTime=currentTime;handleInteractionStart(touch.clientX,touch.clientY)},{passive:false});rndr.domElement.addEventListener("touchmove",e=>{e.preventDefault();if(e.touches.length===1){const touch=e.touches[0];handleInteractionMove(touch.clientX,touch.clientY)}},{passive:false});rndr.domElement.addEventListener("touchend",e=>{e.preventDefault();handleInteractionEnd()},{passive:false});rndr.domElement.addEventListener("dblclick",e=>{e.preventDefault();if(isWithinCanvas(e.clientX,e.clientY)){autoRotate=!autoRotate;if(selectedSprite){selectedSprite=null;if(this.gsap){this.gsap.to(cam.position,{x:0,y:0,z:50,duration:.5,ease:"power2.out"});sps.forEach(sp=>{this.gsap.to(sp.scale,{x:sp.userData.originalScale,y:sp.userData.originalScale,duration:.3})})}}}});function animate(){requestAnimationFrame(animate);if(autoRotate&&!selectedSprite){grp.rotation.y+=.005}grp.children.forEach(s=>s.lookAt(cam.position));rndr.render(scn,cam)}animate();updateCanvasRect()}renderExperience(experiences){const container=document.getElementById("experience-container");container.innerHTML=(experiences||[]).map(exp=>`
            <div class="exp-card" onclick="${exp.companyUrl?`window.open('${exp.companyUrl}', '_blank')`:""}"
                ...
                <div class="card-header-grid">
                    <img src="${exp.logo.startsWith("http")?`https://images.weserv.nl/?url=${encodeURIComponent(exp.logo)}&w=32`:exp.logo}"
                         srcset="${exp.logo.startsWith("http")?`https://images.weserv.nl/?url=${encodeURIComponent(exp.logo)}&w=32 32w, https://images.weserv.nl/?url=${encodeURIComponent(exp.logo)}&w=64 64w`:""}"
                         sizes="32px"
        ...
        `).join("")}renderExtracurriculars(extracurriculars){const container=document.getElementById("extracurricular-container");container.innerHTML=(extracurriculars||[]).map(item=>`
            <div class="exp-card" onclick="${item.url?`window.open('${item.url}', '_blank')`:""}"
                style="cursor: ${item.url?"pointer":"default"}">
                <div class="card-header-grid">
                    <div>
                        ${item.logo?`<img src="https://images.weserv.nl/?url=https://www.jaydeepsolanki.me/${item.logo}&w=32"
                         srcset="https://images.weserv.nl/?url=https://www.jaydeepsolanki.me/${item.logo}&w=32 32w, https://images.weserv.nl/?url=https://www.jaydeepsolanki.me/${item.logo}&w=64 64w"
                         sizes="32px"
                         alt="${item.title||"Activity"} logo" class="companylogo" style="${item.logoStyle||""}" loading="lazy">`:""}
                    </div>
        ...
        `).join("")}renderProjects(projects){const container=document.getElementById("projects-container");const sortedProjects=(projects||[]).sort((a,b)=>{if(a.featured&&!b.featured)return-1;if(!a.featured&&b.featured)return 1;return new Date(b.updatedAt)-new Date(a.updatedAt)}).slice(0,6);container.innerHTML=sortedProjects.map(project=>`
        ...
        `).join("")}renderCertificates(certificates){const container=document.getElementById("certificates-container");container.innerHTML=(certificates||[]).map(cert=>`
        ...
        `).join("")}renderNavigation(navItems){const container=document.getElementById("dock-items");container.innerHTML=(navItems||[]).map((item,index)=>`
        ...
        `).join("")}initializeDock(){if(!this.Veloxi){console.warn("Veloxi not loaded, skipping dock initialization");return}const{createApp,Events,Utils}=this.Veloxi;const MacOsDockPlugin=context=>{let items,root;context.subscribeToEvents(eventBus=>{eventBus.subscribeToEvent(Events.PointerMoveEvent,onMouseMove)});function onMouseMove(event){if(!root.intersects(event.x,event.y)){items.forEach(item=>{item.size.reset()});return}items.forEach(item=>{const progress=Utils.pointToViewProgress({x:event.x,y:event.y},item,120);const scale=Utils.remap(progress,0,1,1,2);item.size.set({width:40*scale,height:40*scale})})}context.setup(()=>{root=context.getView("root");items=context.getViews("item");items.forEach(item=>{item.size.setAnimator("dynamic");item.origin.set({x:.5,y:1})})})};MacOsDockPlugin.pluginName="MacOsDock";const app=createApp();app.addPlugin(MacOsDockPlugin);app.run()}initializeDockHighlighting(){const dockItems=document.querySelectorAll(".macos-dock .item");const sections=[document.getElementById("section1"),document.getElementById("section2"),document.getElementById("section6"),document.getElementById("section3"),document.getElementById("section5"),document.getElementById("section4")].filter(Boolean);if(!sections.length||!dockItems.length)return;let animationTimeout=null;const observerCallback=entries=>{entries.forEach(entry=>{const sectionId=entry.target.id;const dockItem=Array.from(dockItems).find(item=>item.getAttribute("href")===`#${sectionId}`);if(dockItem&&entry.isIntersecting){dockItems.forEach(item=>item.classList.remove("active","section-animate"));dockItem.classList.add("active","section-animate");if(animationTimeout)clearTimeout(animationTimeout);animationTimeout=setTimeout(()=>{dockItem.classList.remove("section-animate")},300)}})};const observerOptions={root:null,rootMargin:"-50% 0px -50% 0px",threshold:0};const observer=new IntersectionObserver(observerCallback,observerOptions);sections.forEach(section=>observer.observe(section))}initializeFadeInAnimations(){const observer=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add("fade-in");observer.unobserve(entry.target)}})},{threshold:.1});const observeNewDivs=()=>{document.querySelectorAll("div:not(.certificate-card):not(.certificate-card-content):not(.certificate-card-content-bottom):not(.loading)").forEach(div=>{if(!div.classList.contains("fade-in")){observer.observe(div)}})};observeNewDivs();const mutationObserver=new MutationObserver(()=>observeNewDivs());mutationObserver.observe(document.body,{childList:true,subtree:true})}initializeExperienceAnimations(){const cards=document.querySelectorAll(".exp-card");const observer=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add("in-view")}})});cards.forEach(card=>{observer.observe(card)})}initializeScrollProgress(){window.addEventListener("scroll",()=>{const scrollTop=document.documentElement.scrollTop||document.body.scrollTop;const scrollHeight=document.documentElement.scrollHeight-document.documentElement.clientHeight;const scrolled=scrollTop/scrollHeight*100;this.scrollProgress.style.setProperty("--scroll-width",`${scrolled}%`)});this.scrollProgress.style.setProperty("--scroll-width","0%")}initializePerformanceMonitoring(){if("PerformanceObserver"in window){try{new PerformanceObserver(entryList=>{for(const entry of entryList.getEntries()){}}).observe({entryTypes:["largest-contentful-paint"]});new PerformanceObserver(entryList=>{for(const entry of entryList.getEntries()){}}).observe({entryTypes:["first-input"]});let clsValue=0;new PerformanceObserver(entryList=>{for(const entry of entryList.getEntries()){if(!entry.hadRecentInput){clsValue+=entry.value}}}).observe({entryTypes:["layout-shift"]})}catch(e){console.warn("Performance monitoring setup failed:",e)}}document.addEventListener("DOMContentLoaded",()=>{console.log("DOM Content Loaded:",performance.now())});window.addEventListener("load",()=>{console.log("Window Load:",performance.now())})}initializeAdaptiveLoading(){if("connection"in navigator){const connection=navigator.connection;const isSlow=connection.effectiveType==="slow-2g"||connection.effectiveType==="2g"||connection.saveData;if(isSlow){document.documentElement.style.setProperty("--transition-medium","0.1s");document.documentElement.style.setProperty("--transition-slow","0.1s");console.log("Slow connection detected - optimizing performance")}}}hideLoading(){this.loadingElement.classList.add("hidden");this.mainContent.classList.remove("hidden");this.dockContainer.classList.remove("hidden");this.isLoading=false}showError(message){this.loadingElement.innerHTML=`<div class="error-message">Error: ${message}</div>`}}const formatDate=dateString=>{if(!dateString)return"N/A";try{const options={year:"numeric",month:"2-digit",day:"2-digit"};return new Date(dateString).toLocaleDateString(undefined,options)}catch(error){console.warn("Invalid date format:",dateString,error);return"N/A"}};document.addEventListener("DOMContentLoaded",()=>{window.portfolioApp=new PortfolioApp});if("serviceWorker"in navigator){window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").then(registration=>{console.log("SW registered: ",registration)}).catch(registrationError=>{console.log("SW registration failed: ",registrationError)})})}window.addEventListener("resize",()=>{setTimeout(()=>{const app=window.portfolioApp;if(app&&typeof app.initializeDockHighlighting==="function"){app.initializeDockHighlighting()}},100)});window.portfolioData={personal:{name:"Jaydeep Solanki",title:"Hello, World! I'm",bio:"Final year B.Tech student in Electronics & Communication Engineering at <a href='https://nirmauni.ac.in/' target='_blank' style='color:rgb(183, 220, 245); text-decoration: underline;'>Nirma University</a>, Ahmedabad. Passionate backend developer specializing in <strong>Python</strong> and building scalable systems. Experienced with FastAPI, PostgreSQL, Docker, and IoT solutions. Currently exploring advanced AI/ML integration, microservices architecture, and cloud-native development.",email:"contactjaydeepsolanki@gmail.com",profileImage:"asset/pfp.jpg",resumeUrl:"asset/cv/Jaydeep_Solanki.pdf",githubUsername:"whoisjayd",hideForks:true,hideArchived:true,university:"Nirma University",degree:"B.Tech in Electronics & Communication Engineering",currentYear:"Final Year (7th Semester)",graduationYear:"2026",location:"Ahmedabad, Gujarat, India"},socialLinks:[{name:"GitHub",url:"https://github.com/whoisjayd",icon:"asset/logos/github.svg"},{name:"LinkedIn",url:"https://www.linkedin.com/in/solanki-jaydeep/",icon:"asset/logos/linkedin.svg"},{name:"Resume",url:"asset/cv/Jaydeep_Solanki.pdf",icon:"asset/logos/resume.svg"},{name:"Twitter",url:"https://twitter.com/JaydeepS14216",icon:"asset/logos/twitter.svg"}],skills:[{name:"LangChain",icon:"asset/logos/langchain.svg",category:"AI/ML"},{name:"Express",icon:"asset/logos/express.svg",category:"Backend"},{name:"FastAPI",icon:"asset/logos/fastapi.svg",category:"Backend"},{name:"Flask",icon:"asset/logos/flask.svg",category:"Backend"},{name:"Node.js",icon:"asset/logos/nodejs.svg",category:"Backend"},{name:"Rest API",icon:"asset/logos/restapi.svg",category:"Backend"},{name:"Azure",icon:"asset/logos/azure.svg",category:"Cloud"},{name:"Google Cloud Platform",icon:"asset/logos/gcp.svg",category:"Cloud"},{name:"MongoDB",icon:"asset/logos/mongo.svg",category:"Database"},{name:"PostgreSQL",icon:"asset/logos/postgresql.svg",category:"Database"},{name:"SQL",icon:"asset/logos/sql.svg",category:"Database"},{name:"Docker",icon:"asset/logos/docker.svg",category:"DevOps"},{name:"Bootstrap",icon:"asset/logos/bootstrap.svg",category:"Frontend"},{name:"CSS",icon:"asset/logos/css.svg",category:"Frontend"},{name:"HTML",icon:"asset/logos/html.svg",category:"Frontend"},{name:"Next.js",icon:"asset/logos/nextjs.svg",category:"Frontend"},{name:"Tailwind CSS",icon:"asset/logos/tailwind.svg",category:"Frontend"},{name:"Arduino",icon:"asset/logos/arduino.svg",category:"Hardware"},{name:"Raspberry Pi",icon:"asset/logos/rpi.svg",category:"Hardware"},{name:"Linux",icon:"asset/logos/linux.svg",category:"Operating System"},{name:"JavaScript",icon:"asset/logos/javascript.svg",category:"Programming"},{name:"Python",icon:"asset/logos/python.svg",category:"Programming"},{name:"TypeScript",icon:"asset/logos/typescript.svg",category:"Programming"},{name:"Selenium",icon:"asset/logos/selenium.svg",category:"Testing"},{name:"Chrome",icon:"asset/logos/chrome.svg",category:"Tools"},{name:"Git",icon:"asset/logos/git.svg",category:"Tools"},{name:"GitHub",icon:"asset/logos/github.svg",category:"Tools"},{name:"VS Code",icon:"asset/logos/vscode.svg",category:"Tools"}],experience:[{role:"Project Intern",company:"Elipsis Infotech",logo:"https://images.weserv.nl/?url=https://www.jaydeepsolanki.me/asset/ellipsis_infotech_logo.jpeg",date:"May 2025 - June 2025",companyUrl:"https://www.linkedin.com/company/ellipsis-infotech/",description:["Developed a real-time AC control system leveraging MQTT and WebSockets to enable seamless communication between ESP8266-based devices and a responsive Next.js frontend, delivering instant device status updates and intuitive control capabilities.","Engineered a secure FastAPI backend with JWT-based authentication and SQLAlchemy for asynchronous database operations, enhancing API performance and ensuring robust management of devices and user profiles.","Implemented an Over-The-Air (OTA) firmware update system for ESP8266 devices using LittleFS, streamlining maintenance and enabling efficient feature deployment with minimal downtime.","Designed a scalable PostgreSQL database schema to support efficient storage and retrieval of device configurations, user data, and historical analytics, optimizing query performance and data integrity.","Crafted a modern, responsive frontend using Next.js, Tailwind CSS, and Shadcn/ui, integrating real-time data visualization and user-friendly controls to enhance user experience and engagement."],skills:["FastAPI","SQLAlchemy","PostgreSQL","MQTT","WebSockets","Next.js","React","TypeScript","Tailwind CSS","Shadcn/ui","JWT","ESP8266","Arduino","LittleFS","RESTful APIs","Database Optimization","Real-time Systems"]}],extracurriculars:[{title:"Team Mentor",date:"SAE, Nirma University | Oct 2024 - Present",description:"Mentoring 15+ team members in troubleshooting hardware and software issues for competitions, while developing strategic approaches to enhance project execution and system performance; actively involved in evaluating team performance, recruiting new members, and maintaining the team website at <a href='https://teamdyaus.space/' target='_blank' style='color: #fff;'>teamdyaus.space</a>.",url:"https://teamdyaus.space/",logo:"asset/logos/teamdyauslogo.png"},{title:"Vice Captain",date:"SAE, Nirma University | Oct 2024 - Present",description:"Coordinated project timelines and task allocation for the Teknofest 2024 Model Satellite Competition, leading the team to secure 9th place among 100+ international teams. Effectively managed financial planning, including detailed expenditure tracking and internal budgeting, ensuring smooth execution of all phases of the project.",url:"https://teamdyaus.space/",logo:"asset/logos/teamdyauslogo.png"}],certificates:[{title:"Model Satellite Competition - Teknofest 2024",issuer:"Teknofest, Turkey",date:"September, 2024",credentialId:"58018",viewUrl:"https://drive.google.com/file/d/1dMDAadEzdYpUXZIoc8hYWNfudpT7McH5/view",image:"asset/certificates/Teknofest_Model_Satellite_Competition.jpg"},{title:"Generative AI & its Use Cases to Boost Your Daily Activities by Amit Thinks",issuer:"Unstop",date:"June, 2024",credentialId:"da4c89e6-0c4d-4cc5-837a-6dc7cdf14868",viewUrl:"https://unstop.com/certificate-preview/da4c89e6-0c4d-4cc5-837a-6dc7cdf14868",image:"asset/certificates/Generative_AI_Certificate.jpg"},{title:"Line Follower - SVNIT Mindbend",issuer:"SVNIT Surat",date:"March, 2024",credentialId:"128a5WOKKJMYip3GnAqbVwP9topVdyH0i",viewUrl:"https://drive.google.com/file/d/128a5WOKKJMYip3GnAqbVwP9topVdyH0i/view",image:"asset/certificates/SVNIT_Mindbend_Line_Follower.png"},{title:"Flipkart Grid 5.0 - Robotics Challenge",issuer:"Flipkart",date:"Jan, 2024",credentialId:"362fef76-85cd-4bcb-9061-dc6169a1aadc",viewUrl:"https://unstop.com/certificate-preview/362fef76-85cd-4bcb-9061-dc6169a1aadc",image:"asset/certificates/Flipart_Grid_5.0.jpg"},{title:"24 Hour Hackathon - MNIT Jaipur",issuer:"MNIT Jaipur",date:"September, 2023",credentialId:"1i32PYf9hW4owhlbLF6ip5AzjtAovtAuH",viewUrl:"https://drive.google.com/file/d/1i32PYf9hW4owhlbLF6ip5AzjtAovtAuH/view",image:"asset/certificates/MNIT_Hackathon.png"},{title:"Smart India Hackathon 2023 - Software Edition",issuer:"E-Cell Nirma University",date:"August, 2023",credentialId:"1h02cv6WoBDrvxt-42UPcOY3VdNI7ISZZ",viewUrl:"https://drive.google.com/file/d/1h02cv6WoBDrvxt-42UPcOY3VdNI7ISZZ/view",image:"asset/certificates/Smart_India_Hackathon_2023.jpg"},{title:"Google Cloud Computing Foundations",issuer:"Techfest, IIT Bombay",date:"December, 2022",credentialId:"159FQkTtd3BTQDw_TV5FLhL_DF6Zi00g5",viewUrl:"https://drive.google.com/file/d/159FQkTtd3BTQDw_TV5FLhL_DF6Zi00g5/view",image:"asset/certificates/IITB_GCP_Certificate.png"}],navigation:[{id:"skills",label:"Skills",icon:"https://www.svgrepo.com/show/376484/earth-sphere-line.svg",section:"section1"},{id:"experience",label:"Experience",icon:"https://www.svgrepo.com/show/483943/old-man-1.svg",section:"section2"},{id:"extracurriculars",label:"Extra-Curriculars",icon:"https://www.svgrepo.com/show/514289/wave.svg",section:"section6"},{id:"projects",label:"Projects",icon:"https://www.svgrepo.com/show/535429/hammer.svg",section:"section3"},{id:"certificates",label:"Certificates",icon:"https://www.svgrepo.com/show/489411/certificate-badge.svg",section:"section5"},{id:"contact",label:"Contact",icon:"https://www.svgrepo.com/show/533193/mail.svg",section:"section4"}]};window.projectOverrides={hiddenProjects:["whoisjayd","Bank-Management-System","teknofest-model-satellite-gui"],hideForks:true,hideArchived:true,projects:[{name:"gitresume",featured:true},{name:"Finance-Manager",featured:true},{name:"teknofest-model-satellite-telemetry",featured:true},{name:"personal-portfolio",featured:true}]};class ContactForm{constructor(){this.form=document.getElementById("contact-form");this.messageElement=document.getElementById("form-message");this.submitButton=this.form.querySelector(".submit-button");this.isSubmitting=false;this.init()}init(){window.emailjs=window.emailjs||{};if(typeof window.emailjs!=="undefined"){window.emailjs.init("xIyOy8I3pQDQwr0nJ")}else{console.warn("EmailJS not loaded")}this.form.addEventListener("submit",this.handleSubmit.bind(this))}async handleSubmit(event){event.preventDefault();if(this.isSubmitting)return;this.isSubmitting=true;this.showMessage("Sending...","");this.setButtonState(true);try{if(typeof window.emailjs==="undefined"){throw new Error("EmailJS not available")}await window.emailjs.sendForm("service_kuwbzdy","template_tnfw7cb",this.form);this.showMessage("Message Sent!","success");this.animateSuccess();setTimeout(()=>{this.resetForm()},2e3)}catch(error){console.error("EmailJS error:",error);this.showMessage("Failed to send message. Please try again.","error")}finally{this.isSubmitting=false;this.setButtonState(false)}}showMessage(text,type){this.messageElement.textContent=text;this.messageElement.className=`form-message ${type}`}setButtonState(disabled){this.submitButton.disabled=disabled;this.submitButton.style.opacity=disabled?"0.6":"1";this.submitButton.style.cursor=disabled?"not-allowed":"pointer"}animateSuccess(){window.gsap=window.gsap||{};if(typeof window.gsap!=="undefined"){window.gsap.fromTo(this.messageElement,{opacity:0,scale:.8,y:20},{opacity:1,scale:1,y:0,duration:.5,ease:"power2.out"})}}resetForm(){this.form.reset();this.messageElement.textContent="";this.messageElement.className="form-message"}}document.addEventListener("DOMContentLoaded",()=>{new ContactForm});(function(f,m){typeof exports=="object"&&typeof module<"u"?m(exports):typeof define=="function"&&define.amd?define(["exports"],m):(f=typeof globalThis<"u"?globalThis:f||self,m(f.Veloxi={}))})(this,function(f){"use strict";var Ct=Object.defineProperty;var It=(f,m,P)=>m in f?Ct(f,m,{enumerable:!0,configurable:!0,writable:!0,value:P}):f[m]=P;var a=(f,m,P)=>(It(f,typeof m!="symbol"?m+"":m,P),P);class m{constructor(e){a(this,"x");a(this,"y");a(this,"target");this.x=e.x,this.y=e.y,this.target=e.target}}class P extends m{}class N extends m{}class C extends m{}class I extends m{}class O{constructor(e){a(this,"pluginId");a(this,"pluginName");a(this,"viewName");a(this,"dataName");a(this,"dataValue");this.event=e,this.pluginId=e.pluginId,this.pluginName=e.pluginName,this.viewName=e.viewName,this.dataName=e.dataName,this.dataValue=e.dataValue}}function Ne(o){return o.replace(/(?:^\w|[A-Z]|\b\w)/g,function(e,t){return t===0?e.toLowerCase():e.toUpperCase()}).replace(/\s+/g,"").replace(/-+/g,"")}function Ce(o){return o.split("").map((e,t)=>e.toUpperCase()===e?`${t!==0?"-":""}${e.toLowerCase()}`:e).join("")}class k{constructor(e){a(this,"node");this.node=e.node}}class z{constructor(e){a(this,"node");this.node=e.node}}class Ie{constructor(e){a(this,"_eventBus");a(this,"_observer");this._eventBus=e,this._observer=new MutationObserver(this._handler.bind(this)),this._observer.observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeOldValue:!0})}_handler(e){e.forEach(t=>{t.addedNodes.forEach(n=>{if(!(n instanceof HTMLElement)||n.dataset.velViewId||n.parentElement&&typeof n.parentElement.dataset.velAdded<"u")return;let r=n;if(n.dataset.velView||(r=n.querySelector("[data-vel-view][data-vel-plugin]")),!r)return;this._eventBus.emitEvent(k,{node:r});const u=r.querySelectorAll("[data-vel-plugin]");u.length&&u.forEach(h=>{this._eventBus.emitEvent(k,{node:h})})}),t.removedNodes.forEach(n=>{if(!(n instanceof HTMLElement)||typeof n.dataset.velProcessing<"u")return;const r=n.querySelectorAll("[data-vel-plugin]");r.length&&r.forEach(u=>{this._eventBus.emitEvent(z,{node:u})}),this._eventBus.emitEvent(z,{node:n})});const i=t.attributeName;if(i&&/data-vel-data-.+/gi.test(i)){const n=t.target,r=n.dataset.velPluginId||"",u=n.dataset.velPlugin||"",h=n.dataset.velView||"",c=n.getAttribute(i);if(c&&c!==t.oldValue){const g=Ne(i.replace("data-vel-data-",""));this._eventBus.emitEvent(O,{pluginId:r,pluginName:u,viewName:h,dataName:g,dataValue:c})}}})}}class Le{execute(e){this.call(e)}}class ie extends Le{constructor(t){super();a(this,"_handler");this._handler=t}call(t){this._handler(t)}}class L{constructor(){a(this,"_listeners",new Map);a(this,"_keyedListeners",new Map)}subscribeToEvent(e,t,i){if(i){this._subscribeToKeyedEvent(e,t,i);return}let s=this._listeners.get(e);s||(s=[],this._listeners.set(e,s)),s.push(new ie(t))}_subscribeToKeyedEvent(e,t,i){let s=this._keyedListeners.get(e);s||(s=new Map,this._keyedListeners.set(e,s));let n=s.get(i);n||(n=[],s.set(i,n)),n.push(new ie(t))}emitEvent(e,t,i){if(i){this._emitKeyedEvent(e,t,i);return}const s=this._listeners.get(e);s&&s.forEach(n=>{n.execute(t)})}_emitKeyedEvent(e,t,i){const s=this._keyedListeners.get(e);if(!s)return;const n=s.get(i);n&&n.forEach(r=>{r.execute(t)})}_convertListener(e){return t=>e(t)}subscribeToPluginReadyEvent(e,t,i=!1){if(i){this.subscribeToEvent(de,this._convertListener(e),t);return}this.subscribeToEvent(ce,this._convertListener(e),t)}emitPluginReadyEvent(e,t,i=!1){if(i){this.emitEvent(de,t,e);return}this.emitEvent(ce,t,e)}reset(){this._listeners.clear()}}let Be=0;function se(){return Be+++""}class ne{constructor(e,t,i,s,n,r,u){a(this,"_registry");a(this,"_eventBus");a(this,"_appEventBus");a(this,"_internalEventBus");a(this,"_initialized",!1);a(this,"_config");a(this,"_pluginFactory");a(this,"_pluginName");a(this,"_id");a(this,"_pluginKey");a(this,"_layoutIdViewMapWaitingToEnter");a(this,"_apiData");a(this,"_isReady",!1);this._id=se(),this._pluginFactory=e,this._pluginName=t,this._registry=i,this._eventBus=s,this._appEventBus=n,this._internalEventBus=new L,this._config=r,this._layoutIdViewMapWaitingToEnter=new Map,this._pluginKey=u,this._apiData={},this._appEventBus.subscribeToPluginReadyEvent(()=>{this._isReady=!0},this._pluginName,!0)}get api(){return this._apiData}_setApi(e){this._apiData=e}get pluginName(){return this._pluginName}get pluginFactory(){return this._pluginFactory}get pluginKey(){return this._pluginKey}get id(){return this._id}get config(){return{...this._config}}getViews(e){return e?this._registry.getViewsByNameForPlugin(this,e):this._registry.getViewsForPlugin(this)}getView(e){return e?this._registry.getViewsByNameForPlugin(this,e)[0]:this._registry.getViewsForPlugin(this)[0]}getViewById(e){return this._registry.getViewById(e)}addView(e){this._registry.assignViewToPlugin(e,this)}setInternalEventBus(e){this._internalEventBus=e}get internalBusEvent(){return this._internalEventBus}emit(e,t){this._internalEventBus.emitEvent(e,t)}on(e,t){this._internalEventBus.subscribeToEvent(e,t)}useEventPlugin(e,t={}){const i=this._registry.createPlugin(e,this._eventBus,t);return this._registry.associateEventPluginWithPlugin(this.id,i.id),i}notifyAboutDataChanged(e){this.onDataChanged(e)}onDataChanged(e){}removeView(e){e.onRemoveCallback?this._invokeRemoveCallback(e):this._deleteView(e),this.onViewRemoved(e)}_invokeRemoveCallback(e){const t=this._createTemporaryView(e);requestAnimationFrame(()=>{var i;(i=t.onRemoveCallback)==null||i.call(t,t,()=>{var s,n;if((s=e.onAddCallbacks)!=null&&s.afterRemoved&&e.layoutId){const r=this._layoutIdViewMapWaitingToEnter.get(e.layoutId);(n=r==null?void 0:r.onAddCallbacks)==null||n.afterEnter(r),this._layoutIdViewMapWaitingToEnter.delete(e.layoutId)}this._deleteView(t,!0)}),setTimeout(()=>{t.element.parentElement&&this._deleteView(t,!0)},1e4)})}_deleteView(e,t=!1){(t||!e.layoutId)&&(this._registry.removeViewById(e.id,this.id),e.element.remove())}_createTemporaryView(e){const t=e.previousRect.viewportOffset,i=e.previousRect.size,s=e.rotation.degrees<0?0:Math.sin(e.rotation.radians)*i.height*e.scale.y,n=e.rotation.degrees>0?0:Math.sin(e.rotation.radians)*i.width*e.scale.y,r=e.element.cloneNode(!0);e.element.remove(),r.style.cssText="",r.style.position="absolute",r.style.left=`${t.left+s}px`,r.style.top=`${t.top-n}px`,r.style.width=`${i.width}px`,r.style.height=`${i.height}px`,r.style.transform=`
